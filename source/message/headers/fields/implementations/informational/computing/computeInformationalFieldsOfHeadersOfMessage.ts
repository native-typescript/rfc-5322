		...
		switch (header.name) {
			...
			case `Keywords`: {
				...
				if (firstKeywordValue === undefined || firstKeywordValue === ``) {
					...
					const phrasesOfKeyword: [Phrase, ...(readonly Phrase[])] = [
					...
					for (const restKeywordValue of restKeywordValues) {
						...
						phrasesOfKeyword.push(new Phrase(rfc2047.decode(restKeywordValue)));
						continue;
					}
					const keyword: Keywords = new Keywords(...phrasesOfKeyword);
					if (keywords === null) {
						keywords = [keyword];
						continue;
					} else {
